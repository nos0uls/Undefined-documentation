...
- Скрывается также при выборе опций (`ChatterboxGetOptionCount > 0`).
- Имя экранируется (`[` → `[lsb]`, `]` → `[rsb]`) перед `scribble(...)`, чтобы случайные скобки в имени не ломали разметку.
- Все настройки лежат в `Create_0.gml` блоком `#region Speaker nameplate settings`:
  - `nameplate_visible`, `nameplate_sprite`
  - `nameplate_offset_x/y` (от центра textbox), `nameplate_anchor_x/y`
  - `nameplate_padding_x/y`, `nameplate_auto_fit_width`, `nameplate_min_width`
  - `nameplate_text_font`, `nameplate_text_scale`, `nameplate_text_color`
  - `nameplate_text_offset_x/y`

### Источник DisplayName (приоритет)
1. Текст из yarn перед `[`.
2. `actor_display_name(code)` — таблица fallback по code.
3. `display_name_prev` — если actor тот же и явного имени нет.
4. Иначе `""` (nameplate скрыт).
...
- сплит `code:emotion` через `string_pos(":")`
- нормализует входные строки
- применяет правила narrator / previous emotion / default
- разрешает `display_name`: yarn → таблица → previous → ""
- возвращает единый struct: `{ actor, emotion, display_name, mouth_closed, mouth_open, sound }`

### `scripts/actor_display_name/actor_display_name.gml`
- статическая таблица fallback display name по `code`
- возвращает `""`, если actor неизвестен
