...
=== "JSON (из `obj_cutsceneTest`)"
    ...
    ??? note "Legacy fallback"
        `obj_cutsceneTest.Step_0` всё ещё поддерживает legacy-функции (`fn`), но все тестовые пункты меню используют JSON (`json`). Каталог тестов строится динамически из `scr_test_catalog()`.
...
=== "Builder (`c_*`)"
    ...
    - `c_speaker("name")` — устанавливает имя говорящего в диалоговом контроллере (`dialogue_controller.speaker_name`).
    - дальнейшие команды `c_setxy`, `c_depth`, `c_facing` требуют явный `target` в первом аргументе: `c_setxy("player", x, y)`.
