...
!!! note "Формат checkpoint полей"
    При импорте engine `.json` поля `include_globals` и `include_instances` ноды `checkpoint_state` ожидаются как JSON-строки (например, `"[\"var1\"]"`). Редактор парсит их обратно в текстовые поля инспектора. Если в файле хранятся нативные массивы — импорт проигнорирует их.
