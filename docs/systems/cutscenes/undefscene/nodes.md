...
### Dialogue Control
Управление поведением активного диалогового окна (`textboxTest_scribble`). Флаги применяются к текущему контроллеру и сохраняются в `obj_cutsceneManager`.

| Параметр | Описание |
|----------|----------|
| prevent_skip | `true` — блокирует скип печати текста кнопкой `confirm`. Подтверждение всё ещё продвигает допечатанную реплику. |
| stay_open | `true` — окно не уничтожается при `ChatterboxIsStopped`. Остаётся открытым до явного `clear_dialogue`. |
| auto_advance | `true` — каждый кадр автоматически подтверждает реплику, диалог проходит без игрока. |

!!! warning "stay_open и Wait for Dialogue"
    При `stay_open = true` `Wait for Dialogue` считает диалог активным, пока окно не закрыто. Закрывайте такой диалог вручную через `Clear Dialogue`.
...
### Branch
Ветвление логики на основе результата функции или скрипта.

| Параметр | Описание |
|----------|----------|
| condition | Имя GML-скрипта/функции, метода или callable-переменной. Возвращаемое значение приводится к `bool`. **Не выражение**. |
...
| stop_when | Условие остановки: `none`, `timeout`, `global_var`, `node_reached` |
| end_var | Переменная для проверки остановки (только `global_var`) |
| end_equals | Ожидаемое значение для остановки (только `global_var`) |
| end_node | Имя ноды для остановки (только `node_reached`) |
| end_timeout | Таймаут остановки в секундах (только `timeout`) |

!!! warning "Глобальные переменные vs флаги"
    `guard_global` читает прямую глобальную переменную `global[var]`, а не `global.flag`. Для установки значения используйте `run_function` с `variable_global_set` или специальный скрипт.
