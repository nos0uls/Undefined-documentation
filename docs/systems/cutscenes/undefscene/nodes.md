...
### Camera Track
Слежение за целью в течение времени или до остановки (`Until Stop`).
| Нода | Параметры |
|------|-----------|
| Camera Track | `target`, `seconds`, `offset_x`, `offset_y` |
| Camera Track Until Stop | `target`, `offset_x`, `offset_y` |
