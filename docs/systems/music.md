...
|------|----------|----------------------|
| `Play Music` | Смена текущего трека | `sound` — ассет музыки; `volume` — громкость (0–1) |
| `Stop Music` | Остановка с фейдом | `fade` — длительность затухания в секундах (по умолчанию 1.0) |
| `Music Volume` | Абсолютная громкость | `volume` — целевое значение (0–1); `fade` — время перехода (по умолчанию 0.5) |
| `Music Duck` | Относительное приглушение | `multiplier` — множитель от текущей громкости (по умолчанию 0.3); `fade` — время перехода (по умолчанию 0.3) |
| `Music Unduck` | Снятие приглушения | `fade` — время возврата полной громкости (по умолчанию 0.3) |
| `Music Pitch` | Изменение скорости воспроизведения | `pitch` — множитель (1.0 = норма) |
| `Music Pause` | Пауза текущего трека | — |
| `Music Resume` | Снятие с паузы | — |

!!! info "Разница между `Music Duck` и `Music Volume`"
    `Music Volume` устанавливает **абсолютную** громкость независимо от настроек игрока.  
    `Music Duck` применяет **относительный** множитель к текущей громкости, уважая общий уровень из настроек. Используйте `Music Duck` для диалогов и катсцен, а `Music Volume` — только когда нужно точное значение.

Подробнее о работе с нодами см. в [Справочнике нод](cutscenes/undefscene/nodes.md).
...
| Поле | Описание | Цветовая индикация |
|------|----------|-------------------|
...
| **Intensity** | Текущая интенсивность слоёв | Зелёный=calm, красный=battle |
| **Prev** | Instance предыдущего трека | Оранжевый при кроссфейде |
| **PAUSED** | Индикатор паузы | Красный |
...
!!! tip "Рекомендуемый формат"
    **OGG Vorbis** — единственный формат, который GameMaker использует для streaming-воспроизведения.

### Нейминг

| Паттерн | Пример | Назначение |
|---------|--------|------------|
| `music_<location>` | `music_school`, `music_menu` | Простой loop |
| `music_<name>_intro` + `music_<name>_loop` | `music_boss_intro`, `music_boss_loop` | Intro + seamless loop |

### Требования к loop

!!! warning "Критично для качества"
